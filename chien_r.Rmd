...
two.way <- aov(TotalFights~as.factor(Year), data=data)
summary(one.way)

interaction <- aov(TotalFights~as.factor(Year), data=data)
summary(one.way)
...
data %>% mutate(fYear=as.factor(Year)) %>% 
  select(TotalFights:fYear) -> data2
# split data
...
# train model
lo.model <- train(TotalFights ~., data = data2, method = "lm",
...
```{r}
#
model_backward <- regsubsets(TotalFights ~ ., data = data2, nvmax = 10, method = "backward")
model.backward.sum <- summary(model_backward)
data.frame(
  Adj.R2 = which.max(model.backward.sum$adjr2),
  CP = which.min(model.backward.sum$cp),
  BIC = which.min(model.backward.sum$bic)
)
